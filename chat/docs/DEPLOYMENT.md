# Chat Deployment - Dev/Prod Environments

## Overview

The Chat chatbot is deployed to Kubernetes with separate dev and prod environments using the **same bot account**. Only one environment runs at a time - you switch between them by deploying to different namespaces.
...
1. Scale prod to 0: `kubectl scale deployment chat-deployment --replicas=0 -n eightbitsaxlounge-prod`
2. Deploy dev version: Push to `dev` branch
3. Test in Twitch chat
4. Scale dev to 0: `kubectl scale deployment chat-deployment --replicas=0 -n eightbitsaxlounge-dev`
5. Scale prod back to 1: `kubectl scale deployment chat-deployment --replicas=1 -n eightbitsaxlounge-prod`
...
1. **Production**: Push to `main` branch or change `chat/version.txt`
   - Builds image: `ghcr.io/owner/eightbitsaxlounge-chat:VERSION`
...
```bash
# Scale prod down
kubectl scale deployment chat-deployment --replicas=0 -n eightbitsaxlounge-prod
...
kubectl scale deployment chat-deployment --replicas=1 -n eightbitsaxlounge-dev
```
...
```bash
# Scale dev down
kubectl scale deployment chat-deployment --replicas=0 -n eightbitsaxlounge-dev

# Scale prod up
kubectl scale deployment chat-deployment --replicas=1 -n eightbitsaxlounge-prod
```
...
kubectl get pods -n eightbitsaxlounge-prod -l app=chat
kubectl get pods -n eightbitsaxlounge-dev -l app=chat
...
```bash
# Production logs
kubectl logs -n eightbitsaxlounge-prod -l app=chat --tail=50 -f

# Development logs
kubectl logs -n eightbitsaxlounge-dev -l app=chat --tail=50 -f
```
...
kubectl describe deployment chat-deployment -n eightbitsaxlounge-prod

# Development
kubectl get pods -n eightbitsaxlounge-dev
kubectl describe deployment chat-deployment -n eightbitsaxlounge-dev
```
...
kubectl rollout restart deployment/chat-deployment -n eightbitsaxlounge-prod

# Restart dev bot
kubectl rollout restart deployment/chat-deployment -n eightbitsaxlounge-dev
```
...
Update the version in `chat/version.txt`:

```bash
# Current version
cat chat/version.txt
...
kubectl logs -n eightbitsaxlounge-prod -l app=chat | grep "expires"
```
...
Check configmap:
```bash
kubectl get configmap chat-config -n eightbitsaxlounge-prod -o yaml
```
